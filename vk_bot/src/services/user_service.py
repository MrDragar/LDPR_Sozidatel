...
class UserService(IUserService):
    ...
    def __init__(self, user_repo: IUserRepository, uow: IUnitOfWork, source: Sources):
    ...
    async def activate_user(
        ...
        async with self.__uow.atomic():
            try:
                tg_user = await self.__user_repo.get_user_by_telegram_id(telegram_id, self.source)
            except UserNotFoundError:
                tg_user = None
            except:
                raise
